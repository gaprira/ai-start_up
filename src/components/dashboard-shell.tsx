    ...
    <div className="min-h-screen grid-bg page-bg">
      {testerMode && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-yellow-500/10 border-b border-yellow-500/20 text-center py-1.5">
          <span className="text-xs font-medium text-yellow-400">{t.shell_tester_mode}</span>
          <button onClick={() => { localStorage.removeItem('testerMode'); window.location.href = '/' }} className="ml-4 text-xs text-yellow-400/70 hover:text-yellow-400 underline">{t.shell_exit}</button>
        </div>
      )}
      <div className={testerMode ? 'pt-8' : ''}>
        ...
        <div className="flex pt-16 max-w-7xl mx-auto">
          <Sidebar />
          <main className="flex-1 p-4 sm:p-8 min-h-[calc(100vh-4rem)]">
            {children}
          </main>
        </div>
      </div>
